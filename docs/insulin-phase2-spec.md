...
- **Learning mode — opt-in ICR + ISF, shipped early 2026-07-20 (`learnedICRs` /
  `learnedISFs`):** at Mark's request (he doses on his own judgement; the app only
  suggests), a conservative subset of component 1 is live ahead of the full
  Phase 2. **Bayesian-shrinkage ICR and ISF per block** blend the seed with clean
  verified 3h outcomes, weighted by count, **hard-bounded to seed·[1 ± 30 %]**.
  With little data they barely move from the seed; they sharpen as outcomes accrue.
  ICR learns from near-range-start meals; **ISF learns from correction-only events**
  (glucose high, carbs ≈ 0, so the 3h drop is the insulin's own effect, not a
  carb+correction mix). Feeds the **SUGGESTED** dose only (never auto-doses), keeps
  every Phase-1 guard, **off by default** (setup toggle). Still to come for full
  Phase 2: per-meal blocks, calibrated confidence bands, backtested auto-apply,
  exercise-sensitivity trim, and endo sign-off.
