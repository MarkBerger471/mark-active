            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  href={item.href}
                  prefetch
                  // Instant tap feel: no 300ms delay, no grey flash, no text
                  // selection, and an immediate press animation. Bigger hit
                  // target (py-2, min-w-[54px]) so taps rarely miss.
                  className={`flex flex-col items-center gap-1 px-3 py-2 rounded-xl min-w-[54px] select-none touch-manipulation transition-transform duration-75 active:scale-90 active:opacity-70 [-webkit-tap-highlight-color:transparent] ${
                    isActive ? 'text-va-red drop-shadow-[0_0_6px_rgba(185,10,10,0.5)]' : 'text-white/45 active:text-white/80'
                  }`}
                >
